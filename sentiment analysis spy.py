...
def smart_score(base,q1,q2,strong):
    ...
    negA=["not","no","never","without","can't","cant","cannot","don't","dont","won't","wont",]
    oppA=[("increase","decrease"),("bigger","smaller"),("more,","less"),("add","remove"),("open","close"),("enalbe","disable")]
    num_pen=0.10 if (nums(q1) and nums(q2) and nums(q1)!=nums(q2)) else 0
    neg_pen=0.12 if has_any(q1.negA)!=has_any(q2,negA)else 0
    opp_pen=0.12 if any((has_any(q1,[a]) and has_any(q2,[b])) or (has_any(q1,[b]) and has_any(q2,[a])) for a,b in oppA) else 0 return max(0.0, min(1.0, 
    base+boost-num_pen-neg_pen-opp_pen))

def label(s): return "✅ DUPLICATE" if s>= TH ("🤒 CLOSE MATCH" if s>=TH-0.05 else "❌ DIFFERENT")
def show_result(s):
    print(f"\n🎯 Result of similarity: {round(s*100,1)}% [{bar(s)}] -> {label(s)}")
    print(f"Rule: score 2 {TH} means DUPLICATE")
    
def show_flow(q1,q2):
    a,b=clean(q1),clean(q2); raw=set(a+b)
    wi={w for w in a if len(w)>=4}; w2={w for w in b if len(w)>=4}
    shared=sorted(w1,w2)
    helpers=sorted({w for w in raw })
...
def main():
    print("Type Question 1 -> Question 2 Then you'll see 2 random demo paris,")
    ...
    try:
        run(q1,q2,"YOUR QUESTION")
        for i,(d1,d2) in enumerate(random,sample(DEMOS,2),1: run(d1,d2,f"RADNOM DEMO{1}"))
